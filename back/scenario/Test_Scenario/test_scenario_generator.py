...
class TestCallLLMWithCurl:
    ...
    @patch("scenario_generator.requests.post")
    def test_appel_reussi(self, mock_post):
    ...
    @patch("scenario_generator.requests.post")
    def test_reponse_vide_leve_exception(self, mock_post):
        mock_response = MagicMock()
        mock_response.text = json.dumps({"response": "", "done": True})
        mock_response.raise_for_status = MagicMock()
        mock_post.return_value = mock_response
    ...
    @patch("scenario_generator.requests.post")
    def test_json_invalide_leve_exception(self, mock_post):
        mock_response = MagicMock()
        mock_response.text = "pas du json valide"
        mock_response.raise_for_status = MagicMock()
        mock_post.return_value = mock_response
        with pytest.raises(ScenarioError):
            call_llm_with_curl("prompt", timeout=10)
    ...
    @patch("scenario_generator.requests.post")
    def test_champ_response_absent_leve_exception(self, mock_post):
        mock_response = MagicMock()
        mock_response.text = json.dumps({"done": True})
        mock_response.raise_for_status = MagicMock()
        mock_post.return_value = mock_response
        with pytest.raises(ScenarioError):
            call_llm_with_curl("prompt", timeout=10)
    ...
    @patch("scenario_generator.requests.post",
           side_effect=__import__("requests").ConnectionError("erreur"))
...
class TestGenerateDialogueFromSegments:
    ...
    def test_genere_script_valide(self, mock_call):
        ...
        assert "Voix_01:" in result["script"]
    ...
    def test_limite_200_lignes(self, mock_call):
        ...
        assert len(result["script"].splitlines()) <= 200
